...
async function main() {
  ...
  const templates = [
    ...
    // --- Finance Shorts templates ---
    {
      ...
      textPrompt:
        'Tell a short finance story. Voice: adapt to what the topic needs — satirical for absurdity, dramatic for stakes, conversational for relatability. Open with one sentence that stops the scroll: a shocking stat, a contrarian claim, or a setup that makes the viewer need to know what happens next ("Two people. Same salary. Same city. One retires at 40."). Then tell the story through escalating scenes — each scene reveals something bigger, more surprising, or more absurd than the last. The viewer should feel like they\'re following a character or subject on a journey, not hearing a list of facts. Stair-step the stakes upward. Save the most jaw-dropping reveal for the very end. End abruptly on the biggest moment — never wind down, never summarize, never say "so remember." Every number must be real and sourceable. Never give financial advice.',
      weight: 50,
      overlayId: sparklesOverlay.id,
      musicId: defaultMusic.id,
    ...
    },
    {
      ...
      textPrompt:
        'Tell a finance data story through escalating reveals. Voice: a dry analyst unveiling something your brain wasn\'t ready for. Open with one sentence that reframes a familiar number into something shocking. Then follow ONE subject — a dollar amount, a person, a purchase, a salary — as it transforms through escalating scenes. Each reveal must be bigger and more visceral than the last: stair-step from relatable ($5 coffee) to uncomfortable ($187K lifetime) to incomprehensible ($4.2 million in lost returns). The viewer should feel like they\'re watching a number grow legs and run away from them. Build to the most extreme scaled consequence at the very end. End abruptly on the biggest number — never wind down. Every number must be traceable to a real source.',
      weight: 50,
      overlayId: sparklesOverlay.id,
      musicId: defaultMusic.id,
