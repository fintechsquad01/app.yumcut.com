...
async function main() {
  ...
  const templates = [
    ...
    // --- Finance Roast Central templates ---
    {
      code: 'finance_roast',
      title: 'Finance Roast',
      description: 'Satirical finance commentary with editorial cartoon energy. Roast bad financial takes, famous investor blunders, meme stocks, and institutional absurdity with data-backed humor.',
      previewImageUrl: '/template/finance-roast/preview.jpg',
      previewVideoUrl: '/template/finance-roast/preview.mp4',
      textPrompt:
        'Write a satirical finance roast. Voice: a data analyst doing late-night stand-up — Bloomberg terminal meets comedy club, zero corporate filter. Open with a single shocking stat or contrarian claim that stops the scroll in one sentence (e.g., "Stop saving money" or "$5 coffee = $187K over 40 years"). Give one line of context explaining why this matters. Then deliver exactly 3 escalating roast beats — each with one real, traceable stat and dry commentary that builds toward the same single thesis. The third beat should be the most absurd. End with a "wait, what?" punchline comparison that reframes everything, then a one-line closer that loops back to the opening hook. Every number must be real and sourceable. Never give financial advice. Roast ideas, institutions, and bad takes — never the viewer.',
      weight: 50,
      overlayId: sparklesOverlay.id,
      musicId: defaultMusic.id,
    ...
    },
    {
      ...
      title: 'Finance Data Viz',
      description: 'Data storytelling through visual metaphors. Wealth scale comparisons, crisis timelines, economic trend reveals, and "the math they never showed you" moments.',
      previewImageUrl: '/template/finance-dataviz/preview.jpg',
      previewVideoUrl: '/template/finance-dataviz/preview.mp4',
      textPrompt:
        'Write narration for a finance data visualization Short. Voice: a dry data analyst revealing something the viewer\'s brain wasn\'t ready for. Open with a single question or reframe stat hook that makes one familiar number mean something completely different (e.g., "The average American will work 40 years and retire with less than $100K. Here\'s the math they were never shown."). All data points in this video must prove the same single thesis — do not introduce multiple topics. Deliver 2-3 escalating stat reveals, each more surprising than the last, with one line of wry commentary per reveal. End with a scaled consequence that makes the abstract number visceral ("That\'s 16,667 years at median salary — before the pyramids were built.") and a one-line closer that recontextualizes the opening. Every number must be traceable to a real source.',
      weight: 50,
      overlayId: sparklesOverlay.id,
      musicId: defaultMusic.id,
    ...
    },
    {
      ...
      description: 'Hypothetical financial chaos with real economic logic. "What if the US printed a $1 trillion coin?", "What if rent was illegal?", "What if Elon ran the Fed?"',
      previewImageUrl: '/template/finance-whatif/preview.jpg',
      previewVideoUrl: '/template/finance-whatif/preview.mp4',
      textPrompt:
        'Write a "what-if" finance scenario Short. Voice: a data analyst imagining an absurd financial reality with satirical glee and real economic logic. Open with one provocative hypothetical that immediately hooks ("What if the US printed a $1 trillion coin?" or "What if rent was illegal?"). Ground the premise in one line of real context. Then walk through exactly 3 escalating real-world consequences — each backed by actual economic data or historical precedent — that logically follow from the premise, getting progressively more absurd. End with the most extreme logical conclusion as the punchline, then a one-line closer that loops back to reality. The tone is playful but the data is deadly serious. Every number must be traceable.',
      weight: 50,
      overlayId: sparklesOverlay.id,
      musicId: defaultMusic.id,
      artStyleId: financeRoastArtStyle?.id,
