...
async function main() {
  ...
  const templates = [
    ...
    // --- Finance Shorts templates ---
    {
      code: 'finance_story',
      title: 'Finance Story',
      description: 'Finance storytelling with editorial cartoon energy. Any finance topic told as a story — satirical, dramatic, educational, or trending. Follows characters and subjects through escalating scenes.',
      previewImageUrl: '/template/finance-story/preview.jpg',
      previewVideoUrl: '/template/finance-story/preview.mp4',
      textPrompt:
        'Tell a short finance story. Voice: adapt to what the topic needs — satirical for absurdity, dramatic for stakes, conversational for relatability. Every concept must be instantly clear to someone who has never read a finance article. Open with one sentence that stops the scroll: a shocking stat, a contrarian claim, or a setup that makes the viewer need to know what happens next ("Two people. Same salary. Same city. One retires at 40."). Then tell the story through escalating scenes — each scene reveals something bigger, more surprising, or more absurd than the last. Each scene must open with a mini-hook that re-earns attention ("But here\'s where it gets worse," "And they weren\'t done"). The viewer should feel like they\'re following a character or subject on a journey, not hearing a list of facts. Stair-step the stakes upward. Save the most jaw-dropping reveal for the very end. End abruptly on the biggest moment — never wind down, never summarize, never say "so remember." Every number must be real and sourceable. Never give financial advice.',
      weight: 50,
      overlayId: sparklesOverlay.id,
      musicId: defaultMusic.id,
    ...
    {
      code: 'finance_reveal',
      title: 'Finance Reveal',
      description: 'Data storytelling through escalating reveals. Follow one number on a stair-stepping journey from relatable to incomprehensible — $5 coffee to $4.2 million in lost returns.',
      previewImageUrl: '/template/finance-reveal/preview.jpg',
      previewVideoUrl: '/template/finance-reveal/preview.mp4',
      textPrompt:
        'Tell a finance data story through escalating reveals. Voice: a dry analyst unveiling something your brain wasn\'t ready for. Use zero financial jargon — translate every concept into physical, everyday language. Open with one sentence that reframes a familiar number into something shocking. Then follow ONE subject — a dollar amount, a person, a purchase, a salary — as it transforms through escalating scenes. Each reveal must be bigger and more visceral than the last: stair-step from relatable ($5 coffee) to uncomfortable ($187K lifetime) to incomprehensible ($4.2 million in lost returns). Each reveal must open with a re-hook phrase that makes the viewer lean in again ("But here\'s the number nobody talks about," "Now scale that up"). The viewer should feel like they\'re watching a number grow legs and run away from them. Build to the most extreme scaled consequence at the very end. End abruptly on the biggest number — never wind down. Every number must be traceable to a real source.',
      weight: 50,
      overlayId: sparklesOverlay.id,
      musicId: defaultMusic.id,
    ...
    {
      code: 'finance_scenario',
      title: 'Finance Scenario',
      description: 'What-if finance stories with real economic logic. "What if rent was illegal?", "What if the US printed a $1 trillion coin?" — hypotheticals that escalate into real consequences.',
      previewImageUrl: '/template/finance-scenario/preview.jpg',
      previewVideoUrl: '/template/finance-scenario/preview.mp4',
      textPrompt:
        'Tell a "what-if" finance story. Voice: a data analyst imagining an absurd reality with real economic logic. Explain every consequence in language a non-finance viewer would immediately understand. Open with one provocative hypothetical that makes the viewer stop scrolling. Then tell the story of what actually happens — walk scene by scene through escalating consequences, each backed by real data or historical precedent. Each consequence must open with a mini-hook that re-earns attention ("But that\'s just the start," "Now here\'s where it breaks"). The viewer should feel like they\'re watching a chain reaction: each consequence triggers the next, getting progressively more extreme. Stair-step from plausible to insane. The most extreme logical conclusion is the payoff at the very end. End abruptly — never return to reality, never reflect. Every number must be traceable.',
      weight: 50,
      overlayId: sparklesOverlay.id,
      musicId: defaultMusic.id,
      artStyleId: financeRoastArtStyle?.id,
